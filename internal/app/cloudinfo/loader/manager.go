...
func (sm *defaultServiceManager) LoadServiceInformation(ctx context.Context, providers []string) {
	...
	for _, provider := range providers {
		...
		for _, service := range sm.services[provider] {
			if !service.IsStatic {
				sm.log.Debug("skip loading static cloud information, dynamic service", map[string]interface{}{"service": service})
			}

			cloudInfoLoader := NewCloudInfoLoader(service.DataLocation, service.DataLocation, sm.store, sm.log)
			// todo async?
...
func NewDefaultServiceManager(config Config, store cloudinfo.CloudInfoStore, log logur.Logger) ServiceManager {
